...
log_file="/home/chaturvedakash1/server_log.log"
storage_file="/home/chaturvedakash1/store.txt"
storage_backup="/home/chaturvedakash1/store_bkp.txt"
 ...
 try:
    ...
    for i in st:
        ...
        if l[0] in d:
            ...
            d[l[0]]=l[1]
    words_len=len(d)
    size1=math.ceil(words_len//3)
    i = iter(d.items())
    d1 = dict(itertools.islice(i,size1))
    d2= dict(itertools.islice(i,size1))
    d3 = dict(i)
    st=""
    for i in d1:
        st+="("+i+"::"+str(d1.get(i))+")"
...
def backup():
 ...
 try:
     if(os.path.exists(storage_file)):
         lock.acquire()
         os.rename(storage_file,storage_backup)
         lock.release()
         res=True
 except Exception as e:
     logging.error("Exception occurred", exc_info=True)
